package net.primegames.supremeprison.autominer.model;
...
public final class AutoMinerTask implements Runnable {
    ...
    private void decrementPlayerAutoMinerTimeAndNotify(Player p) {
        ...
        p.spigot().sendMessage(ChatMessageType.ACTION_BAR, new TextComponent(this.region.getPlugin().getAutoMinerConfig().getMessage("auto_miner_enabled")));
    }

    private boolean executeTaskValidationAndNotifyPlayerOnFail(Player p) {

        if (!this.checkPlayerAutoMinerTime(p)) {
            p.spigot().sendMessage(ChatMessageType.ACTION_BAR, new TextComponent(this.region.getPlugin().getAutoMinerConfig().getMessage("auto_miner_disabled")));
            return false;
        }

        if (!this.checkPlayerItemInHand(p)) {
            p.spigot().sendMessage(ChatMessageType.ACTION_BAR, new TextComponent(this.region.getPlugin().getAutoMinerConfig().getMessage("auto_miner_no_pickaxe")));
            return false;
        }
