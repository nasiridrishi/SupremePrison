package net.primegames.supremeprison.mainmenu;

import net.primegames.supremeprison.SupremePrison;
import net.primegames.supremeprison.history.gui.PlayerHistoryGUI;
import net.primegames.supremeprison.mainmenu.reload.ReloadSelectionGui;
import net.primegames.supremeprison.mainmenu.reset.ResetSelectionGui;
...
public class MainMenu extends Gui {
    ...
    public MainMenu(SupremePrison core, Player player) {
    ...
    public void redraw() {
        ...
        this.setItem(21, ItemStackBuilder.of(SkullUtils.COMMAND_BLOCK_SKULL.clone()).name("&e&lReload Modules").lore("&7Click to reload specific module").build(() -> {
            if (!this.getPlayer().hasPermission("supremeprison.mainmenu.reload")) {
                return;
            }
            this.close();
        ...
        this.setItem(22, ItemStackBuilder.of(this.core.isDebugMode() ? SkullUtils.CHECK_SKULL.clone() : SkullUtils.CROSS_SKULL.clone()).name("&e&lDebug Mode: " + (this.core.isDebugMode() ? "&2&lON" : "&c&lOFF")).lore("&7Click to toggle debug mode.").build(() -> {
            if (!this.getPlayer().hasPermission("supremeprison.mainmenu.debug")) {
                return;
            }
        ...
        this.setItem(23, ItemStackBuilder.of(SkullUtils.DANGER_SKULL.clone()).name("&e&lReset Player Data").lore("&7Click to select which module data", "&7would you like to wipe.").build(() -> {
            if (!this.getPlayer().hasPermission("supremeprison.mainmenu.reset")) {
                return;
            }
            this.close();
        ...
        this.setItem(31, ItemStackBuilder.of(CompMaterial.BOOK.toItem()).name("&e&lPlayers History").lore("&7Click to see players history.").build(() -> {
            if (!this.getPlayer().hasPermission("supremeprison.mainmenu.history")) {
                return;
            }
            this.close();
