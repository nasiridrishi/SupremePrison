package net.primegames.supremeprison.mines.model.mine;
...
import net.primegames.supremeprison.mines.model.mine.reset.ResetType;
...
public class Mine implements GsonSerializable {
    ...
    public boolean isInMine(Location loc) {
        return this.mineRegion.contains(BukkitAdapter.asBlockVector(loc));
    }

    private int calculateCurrentBlocks() {
        Vector3 min = this.mineRegion.getMinimumPoint().toVector3();
        Vector3 max = this.mineRegion.getMaximumPoint().toVector3();
        ...
        for (int x = minX; x <= maxX; x++) {
            for (int y = minY; y <= maxY; y++) {
                for (int z = minZ; z <= maxZ; z++) {
                    BlockState b = mineRegion.getWorld().getBlock(BlockVector3.at(x, y, z));
    ...
    public JsonElement serialize() {
        ...
        builder.addIfAbsent("region", MineSelection.fromWeRegion(this.mineRegion));
    ...
    public boolean canTeleport(Player player) {
        return player.hasPermission("supremeprison.mines.tp." + this.name);
    }
    ...
    public void onPostReset() {
        if (resetTime > 0) {
            this.nextResetDate = new Date(System.currentTimeMillis() + TimeUnit.MINUTES.toMillis(resetTime));
        } else {
            this.nextResetDate = null;
        }
        this.updateCurrentBlocks();
        this.setResetting(false);
        SupremePrison.getInstance().getLogger().info("MineReset: Resetting Mine " + this.getName() + " completed");
        updateHolograms();
    }
    ...
    public static final class Builder {
        ...
        public Mine build() {
            return new Mine(SupremePrisonMines.getInstance().getManager(), this.name, this.mineRegion, this.teleportLocation, this.blockPalette, this.resetType, this.resetPercentage, this.broadcastReset, this.blocksLeftHologram, this.blocksMinedHologram, this.timedResetHologram, this.mineEffects, this.timedReset);
        }
