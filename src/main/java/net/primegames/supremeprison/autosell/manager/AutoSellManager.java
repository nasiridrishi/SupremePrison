package net.primegames.supremeprison.autosell.manager;

import net.primegames.supremeprison.SupremePrison;
import net.primegames.supremeprison.autosell.SupremePrisonAutoSell;
import net.primegames.supremeprison.autosell.api.events.SupremePrisonAutoSellEvent;
import net.primegames.supremeprison.autosell.api.events.SupremePrisonSellAllEvent;
...
import net.primegames.supremeprison.utils.economy.EconomyUtils;
import net.primegames.supremeprison.utils.inventory.InventoryUtils;
import net.primegames.supremeprison.utils.misc.MaterialUtils;
...
public class AutoSellManager {
    ...
    public AutoSellManager(SupremePrisonAutoSell plugin) {
        this.plugin = plugin;
    ...
    private boolean loadSellRegionFromConfig(YamlConfiguration config, String regionName) {
        ...
        if (worldName == null) {
            throw new NullPointerException("Could not load world name for region " + regionName + "!");
        }
    ...
    public void sellAll(Player sender, SellRegion sellRegion) {
        if (!checkIfPlayerCanSellInSellRegion(sender, sellRegion)) {
            SupremePrison.getInstance().getLogger().info("player can't sell in sell region: " + sellRegion.getRegion().getId());
            return;
        }

        Map<AutoSellItemStack, Double> itemsToSell = sellRegion.previewInventorySell(sender);

        SupremePrisonSellAllEvent event = this.callSellAllEvent(sender, sellRegion, itemsToSell);
        ...
        if (totalAmount > 0.0) {
            ...
            PlayerUtils.sendMessage(sender, this.plugin.getAutoSellConfig().getMessage("sell_all_no_items"));
        }
    }

    public void sellAll(Player sender) {
        String sellRegion = null;
        this.sellAll(sender, sellRegion);
    }

    public void sellAll(Player sender, @Nullable String region) {
        SellRegion sellRegion = null;

        if (region != null) {
            sellRegion = this.regionsAutoSell.get(region);
        } else {
            for (SellRegion sellRegion1 : this.regionsAutoSell.values()) {
                if (sellRegion1.contains(sender.getLocation())) {
                    sellRegion = sellRegion1;
                    break;
                }
            }
        }
        ...
        if (sellRegion == null) {
            PlayerUtils.sendMessage(sender, "&3[&bSellAll&3] &cYou are not in a sell region! You must be in one of the mine to use this command!");
            return;
        }

        sellAll(sender, sellRegion);
    ...
    private SupremePrisonSellAllEvent callSellAllEvent(Player sender, SellRegion sellRegion, Map<AutoSellItemStack, Double> sellItems) {
        SupremePrisonSellAllEvent event = new SupremePrisonSellAllEvent(sender, sellRegion, sellItems);

        Events.call(event);

        if (event.isCancelled()) {
            this.plugin.getCore().debug("SupremePrisonSellAllEvent was cancelled.", this.plugin);
        }

        return event;
    }

    private SupremePrisonAutoSellEvent callAutoSellEvent(Player player, SellRegion sellRegion, Map<AutoSellItemStack, Double> itemsToSell) {
        SupremePrisonAutoSellEvent event = new SupremePrisonAutoSellEvent(player, sellRegion, itemsToSell);

        Events.call(event);

        if (event.isCancelled()) {
            this.plugin.getCore().debug("SupremePrisonAutoSellEvent was cancelled.", this.plugin);
        }
    ...
    public boolean autoSellBlock(Player player, Block block) {
        ...
        SupremePrisonAutoSellEvent event = this.callAutoSellEvent(player, sellRegion, itemsToSell);
    ...
    public double getPriceForBlock(Location location, Block block) {
        CompMaterial material = CompMaterial.fromBlock(block);
        SellRegion region = getAutoSellRegion(location);
        if (region != null) {
            return region.getSellPriceForMaterial(material);
        }
        return 0.0;
