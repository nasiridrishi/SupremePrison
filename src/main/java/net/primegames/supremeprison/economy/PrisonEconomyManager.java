...
import net.primegames.supremeprison.economy.event.PlayerBalanceReceiveEvent;
import net.primegames.supremeprison.economy.event.PlayerBalanceRemoveEvent;
...
@Getter
public class PrisonEconomyManager implements SupremePrisonModule {
    ...
    public void addBalance(PrisonCurrency currency, Player player, double amount) {
        ...
        SupremePrison.getInstance().getLogger().log(Level.INFO, "Added " + event.getAmount() + " " + currency.getName() + " to " + player.getName());
    }
    ...
    public void takeBalance(PrisonCurrency currency, OfflinePlayer player, double amount) {
        ...
        SupremePrison.getInstance().getLogger().log(Level.INFO, "Took " + event.getAmount() + " " + currency.getName() + " from " + player.getName());
    }
    ...
    private Account getAccount(OfflinePlayer player) {
        ...
        if (!optAccount.isPresent()) {
            SupremePrison.getInstance().getLogger().log(Level.SEVERE, "Account not found for player " + player.getName(), new IllegalStateException("Account not found!"));
            return null;
        }
    ...
    public void setBalance(PrisonCurrency prisonCurrency, OfflinePlayer p, double newAmount) {
        ...
        if (account == null) {
            SupremePrison.getInstance().getLogger().log(Level.SEVERE, "Account not found for player " + p.getName(), new IllegalStateException("Account not found!"));
            return;
        }
    ...
    public void enable() {
        ...
        if (!missingCurrencies.isEmpty()) {
            SupremePrison.getInstance().getLogger().log(Level.SEVERE, "Missing currencies: " + missingCurrencies);
            ArrayList<String> currencies = new ArrayList<>();
            for (Currency currency : UltraEconomy.getAPI().getCurrencies()) {
                currencies.add(currency.getName());
            }
            SupremePrison.getInstance().getLogger().log(Level.SEVERE, "Available currencies: " + currencies);
            SupremePrison.getInstance().getLogger().log(Level.SEVERE, "Disabling plugin...");
            Bukkit.getPluginManager().disablePlugin(SupremePrison.getInstance());
        }
    }
