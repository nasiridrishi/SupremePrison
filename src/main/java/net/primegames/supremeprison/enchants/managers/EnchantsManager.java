...
import net.primegames.supremeprison.enchants.api.events.SupremePrisonPlayerEnchantEvent;
...
public class EnchantsManager {
    ...
    public EnchantsManager(SupremePrisonEnchants plugin) {
        this.plugin = plugin;
        this.lockedPlayers = Collections.synchronizedList(new ArrayList<>());
    }

    public Map<SupremePrisonEnchantment, Integer> getItemEnchants(ItemStack itemStack) {

        Map<SupremePrisonEnchantment, Integer> returnMap = new HashMap<>();
        ...
        for (SupremePrisonEnchantment enchantment : getEnchantsRepository().getAll()) {
    ...
    private ItemStack applyLoreToPickaxe(Player player, ItemStack item) {
        ...
        boolean pickaxeLevels = this.plugin.getCore().isModuleEnabled(SupremePrisonPickaxeLevels.MODULE_NAME);
        ...
        Map<SupremePrisonEnchantment, Integer> enchants = this.getItemEnchants(item);
        ...
        for (String s : pickaxeLore) {
            ...
            if (matcher.find()) {
                ...
                SupremePrisonEnchantment enchantment = getEnchantsRepository().getEnchantById(enchId);
    ...
    public synchronized int getEnchantLevel(ItemStack itemStack, SupremePrisonEnchantment enchantment) {
    ...
    public void handleBlockBreak(BlockBreakEvent e, ItemStack pickAxe) {
        ...
        Map<SupremePrisonEnchantment, Integer> playerEnchants = this.getItemEnchants(pickAxe);

        for (SupremePrisonEnchantment enchantment : playerEnchants.keySet()) {
    ...
    public void handlePickaxeEquip(Player p, ItemStack newItem) {
        Map<SupremePrisonEnchantment, Integer> playerEnchants = this.getItemEnchants(newItem);
        for (SupremePrisonEnchantment enchantment : playerEnchants.keySet()) {
            if (!enchantment.isEnabled()) {
                continue;
            }
    ...
    public void buyEnchnant(SupremePrisonEnchantment enchantment, EnchantGUI gui, int currentLevel, int addition) {
        ...
        if (!PrisonEconomyManager.getInstance().hasBalance(PrisonCurrency.TOKEN, gui.getPlayer(), totalCost)) {
            PlayerUtils.sendMessage(gui.getPlayer(), plugin.getEnchantsConfig().getMessage("not_enough_tokens"));
            return;
        }

        SupremePrisonPlayerEnchantEvent event = new SupremePrisonPlayerEnchantEvent(gui.getPlayer(), totalCost, currentLevel + addition);
        ...
        PrisonEconomyManager.getInstance().takeBalance(PrisonCurrency.TOKEN, gui.getPlayer(), totalCost);
    ...
    public void disenchant(SupremePrisonEnchantment enchantment, DisenchantGUI gui, int currentLevel, int substraction) {
        ...
        PrisonEconomyManager.getInstance().addBalance(PrisonCurrency.TOKEN, gui.getPlayer(), totalRefunded);
    ...
    public void disenchantMax(SupremePrisonEnchantment enchantment, DisenchantGUI gui, int currentLevel) {
    ...
    public void buyMaxEnchant(SupremePrisonEnchantment enchantment, EnchantGUI gui, int currentLevel) {
        ...
        Schedulers.async().run(() -> {
            ...
            while (gui.getPlayer().isOnline() && (currentLevel + levelsToBuy + 1) <= enchantment.getMaxLevel() && PrisonEconomyManager.getInstance().hasBalance(PrisonCurrency.TOKEN, gui.getPlayer(), totalCost + enchantment.getCostOfLevel(currentLevel + levelsToBuy + 1))) {
                levelsToBuy += 1;
            ...
            SupremePrisonPlayerEnchantEvent event = new SupremePrisonPlayerEnchantEvent(gui.getPlayer(), totalCost, currentLevel + levelsToBuy);
            ...
            PrisonEconomyManager.getInstance().addBalance(PrisonCurrency.TOKEN, gui.getPlayer(), totalCost);
    ...
    public void givePickaxe(Player target, Map<SupremePrisonEnchantment, Integer> enchants, String pickaxeName, CommandSender sender) {
        ...
        for (Map.Entry<SupremePrisonEnchantment, Integer> entry : enchants.entrySet()) {
    ...
    public ItemStack createFirstJoinPickaxe(Player player) {
        ...
        for (String s : firstJoinPickaxeEnchants) {
            try {
                String[] data = s.split(" ");
                SupremePrisonEnchantment enchantment = getEnchantsRepository().getEnchantByName(data[0]);
