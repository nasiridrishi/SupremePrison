...
@Getter
public abstract class SupremePrisonEnchantment implements Refundable {

    protected final SupremePrisonEnchants plugin;
    ...
    public SupremePrisonEnchantment(SupremePrisonEnchants plugin, int id) {
        this.plugin = plugin;
        this.id = id;
    ...
    public boolean canBeBought(ItemStack pickAxe) {
        if (!this.plugin.getCore().isModuleEnabled(SupremePrisonPickaxeLevels.MODULE_NAME)) {
            return true;
        }
