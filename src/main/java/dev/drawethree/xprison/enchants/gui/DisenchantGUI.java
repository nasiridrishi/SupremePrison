...
public final class DisenchantGUI extends Gui {
    ...
    public static void init() {
        ...
        EMPTY_SLOT_ITEM = ItemStackBuilder.
                of(CompMaterial.fromString(XPrisonEnchants.getInstance().getEnchantsConfig().getYamlConfig().getString("disenchant_menu.empty_slots")).toItem()).name("&e").buildItem().build();
        ...
        if (HELP_ITEM_ENABLED) {
            HELP_ITEM = ItemStackBuilder.of(SkullUtils.HELP_SKULL).name(Objects.requireNonNull(XPrisonEnchants.getInstance().getEnchantsConfig().getYamlConfig().getString("disenchant_menu.help_item.name"))).lore(XPrisonEnchants.getInstance().getEnchantsConfig().getYamlConfig().getStringList("disenchant_menu.help_item.lore")).buildItem().build();
